...
type RequestPipeline func(*ComponentsContext)

var emptyPipeline RequestPipeline = func(*ComponentsContext) { /* do nothing */ }
...
func CreatePipeline(components ...interface{}) RequestPipeline {
	...
	for i := len(components) - 1; i >= 0; i-- {
		...
		if servComp, ok := currentComponent.(ServicesMiddlwareComponent); ok {
			...
			f = func(context *ComponentsContext) {
...
func createServiceDependentFunction(component interface{},
	...
	if method.IsValid() {
		return func(context *ComponentsContext) {
			if context.error == nil {
