...
func NewRouter(handler ...HandlerEntry) *RouterComponent {
	routes := generateRoutes(handler...)

	var urlGen URLGenerator
	services.GetService(&urlGen)
	if urlGen == nil {
		services.AddSingleton(func() URLGenerator {
			return &routeUrlGenetator{routes: routes}
		})
	} else {
...
func (router *RouterComponent) ProcessRequest(context *pipeline.ComponentsContext, next func(componentsContext *pipeline.ComponentsContext)) {
...
func (router *RouterComponent) invokeHandler(route Route, rawParams []string, context *pipeline.ComponentsContext) error {
