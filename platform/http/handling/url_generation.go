...
type routeUrlGenetator struct {
	routes []Route
}

func (gen *routeUrlGenetator) AddRoutes(routes []Route) {
	if gen.routes == nil {
		gen.routes = routes
	} else {
...
func (gen *routeUrlGenetator) GenerateUrl(method interface{}, data ...interface{}) (string, error) {
...
func (gen *routeUrlGenetator) GenerateURLByName(handlerName, methodName string, data ...interface{}) (string, error) {
	for _, route := range gen.routes {
...
func GenerateUrl(route Route, data ...interface{}) (url string, err error) {
	...
	if len(data) > 0 && !strings.EqualFold(route.httpMethod, http.MethodGet) {
		err = errors.New("Onle GET handler can have data values")
	} else if strings.EqualFold(route.httpMethod, http.MethodGet) && len(data) != route.handlerMethod.Type.NumIn()-1 {
		err = errors.New("Number of data values doesnt match method params")
	} else {
